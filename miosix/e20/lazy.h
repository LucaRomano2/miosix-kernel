...
public:
    ...
    static LazyScheduler& instance();

    LazyScheduler(const LazyScheduler&) = delete;
    LazyScheduler& operator= (const LazyScheduler&) = delete;

private:
    std::list<Thread*> events[NUM_PRIORITIES]; ///< Event queue
    ...
    void lazy_execute(future_base* fut, int prio);
};

template<class F, class... Args>
lazy::future<typename std::result_of<F(Args...)>::type>* async(F&& f, Args&&... args)
{
